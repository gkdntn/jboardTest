...
public class SQL {
	...
	public static final String SELECT_ARTICLES = "SELECT ROW_NUMBER() OVER(ORDER BY `no` DESC)"
													+ " ,a.*, b.nick  FROM `article` AS a "
	...
	public static final String SELECT_COMMENT = "select a.*, b.nick from `comment` as a join `user` as b on a.writer = b.uid where `no`=?";
